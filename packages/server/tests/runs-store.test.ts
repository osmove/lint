...
describe("createRunsStore", () => {
  let tmpDir: string;
  const workspace = (root: string) => ({ root, name: path.basename(root) });
  ...
  it("returns empty list when no .lint/runs.jsonl exists", () => {
    const store = createRunsStore(workspace(tmpDir));
  ...
  it("update appends a new line, last-write-wins on read", () => {
    ...
    // The underlying file should now have two lines (insert + update).
  ...
  it("survives malformed lines (best-effort parse)", () => {
    ...
    fs.writeFileSync(file, `{"id":"good","startedAt":"2026-01-01T00:00:00.000Z","errorCount":0,"warningCount":0,"status":"passed"}\nNOT JSON\n`, "utf-8");
    const store = createRunsStore(workspace(tmpDir));
