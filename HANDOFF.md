...
Status as of **April 30, 2026**. Everything below was scaffolded across multiple sessions starting from a monorepo with just `packages/cli`. The goal was to mirror the Backlog topology (CLI + SDK + Server + Dashboard UI + Desktop) and ship it production-grade.

## TL;DR

- 13 packages, 124 tests passing, all builds green.
...
- Multi-repo registry at `~/.lint/projects.json`, exposed via CLI + REST.
...
├── lint-cloud/           Rails 7.2 backend (lint.to / api.lint.to) — pre-existing
...
```
   ...
   .lint/config ──┼──► @lint/config ─┐  fetches /api/* via │
                   └──►              │  @osmove/lint-sdk    │
   ~/.lint/projects.json ──► @lint/core (registry)    ▲    │
                                                      │    │
...
|---|---|---|
| `cli` | ✅ Production | All commander commands work. `lint repos {list,add,rm}` recently added. CLI writes runs to `.lint/runs.jsonl`. |
| `sdk` | ✅ Published-ready | OpenAPI types regenerate via `pnpm --filter @osmove/lint-sdk openapi:types`. Spec at `packages/sdk/openapi/v1.yaml` and mirrored at `lint-cloud/public/openapi/v1.yaml`. |
| `server` | ✅ Production | Hono on `:7878`. Routes: `/api/{health,runs,policies,repos,linters,ai}`. POST /api/runs spawns lint, emits SSE. |
...
- **124 tests passing** across 13 packages
- breakdown: config 25, linters 24, git 15, policies 7, core 49 (orchestrator/doctor/reporter/detect/git/runs-store/projects-registry), cli 4
...
```
lint-cli (last 12 commits, all signed jimdou):
...
- [ ] **`lint-cloud/public/openapi/v1.yaml` is currently a static copy.** Long-term it should be generated by the Rails app (rswag or grape-swagger). Confirm with the user before doing this — the static path is fine for v1.

### P1 — Real value, ~half-day each

- [ ] **Latest-run-per-repo index for `/api/repos/:id/health`.** Currently returns `status: "unknown", lastRun: null`. Should read the most recent passed/failed run from `.lint/runs.jsonl` (per repo root). Adds a `getLatestRun(workspaceRoot)` to `@lint/core/runs-store.ts`. Surfaces in the dashboard's Repos tab as colored badges.
- [ ] **GitHub Checks integration**. When `lint ci` runs in a GitHub Action, post a Check Run via `@octokit/rest`. Adds annotations on the failing files. New env: `GITHUB_TOKEN` already present in Actions; no new secret. Implementation: extend `lint ci` command to detect GH context (`GITHUB_ACTIONS=true`) and call the Checks API.
- [ ] **Slack/Discord notifications on `failed` runs.** Add `notify:` block to `.lintrc.yaml`:
  ```yaml
  notify:
    on: [failed]
    slack: ${SLACK_WEBHOOK}
  ...
  ```
  After `recordRun("failed", ...)` in the orchestrator, fire a webhook with run summary + repo + ref. Best-effort, swallow errors.
- [ ] **Workspace picker uses the registry.** Currently `packages/desktop/src/workspace-picker.ts` shows the OS dialog every time and stores only the last one. Should read `~/.lint/projects.json` and present a recent-list first ("Open" / "Open another…"). Falls through to `dialog.showOpenDialog` when the user picks "Open another".
